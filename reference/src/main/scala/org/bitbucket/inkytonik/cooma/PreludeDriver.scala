...
class PreludeDriver extends Driver {
    ...
    /**
     * Read the prelude and write the static and dynamic information files.
     * The static file contains names, types and values for prelude entities.
     * The dynamic file contains IR code for the whole prelude.
     */
        ...
        for (scope <- env.reverse) {
            for ((id, entity) <- scope) {
                analyser.entityType(entity) match {
                    ...
                    case None =>
                        sys.error(s"writeStaticPrelude: can't find type for $id entity $entity")
                }
            }
        }
    ...
    def writeDynamicPrelude(source : Source, program : Program, config : Config) : Unit = {
