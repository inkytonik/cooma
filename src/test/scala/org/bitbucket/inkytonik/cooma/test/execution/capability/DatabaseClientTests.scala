...
class DatabaseClientTests extends ExecutionTests {
  ...
  def run(filename: String, databases: Int*)(implicit
      bc: BackendConfig
  ): String = {
    val now = System.currentTimeMillis()
    val paths = databases
      .map(i => (s"$basePath/test_$i.db", s"$basePath/test_${i}_$now.db"))
      .toSeq
    paths.foreach { case (base, copy) =>
      Files.copy(Paths.get(base), Paths.get(copy))
    }
