...
class TopLevelRewriterTests
  ...
  object TopLevelRewriterOnly {
    ...
    def test(
        name: String,
        source: String,
        expected: Either[Seq[String], String]
    ): Unit =
  ...
  object WithSemanticAnalysis extends SemanticTests {
    ...
    override def test(
      ...
      TopLevelRewriterTests.this.test(name) {
        ...
        ) shouldBe expectedMessages.stripMargin
      }
  ...
  def test(
      name: String,
      source: String,
      expected: Either[(String, Seq[String]), String]
  ): Unit = {
    TopLevelRewriterOnly.test(
      s"[TopLevelRewriter] $name",
      source,
      expected.left.map(_._2)
    )
    WithSemanticAnalysis.test(
      s"[SemanticAnalyser] $name",
      source,
      expected match {
        case Right(_)           => ""
  ...
  test(
    "do not rewrite a program consisting of a non-block expression",
    "0",
    "0"
  )
