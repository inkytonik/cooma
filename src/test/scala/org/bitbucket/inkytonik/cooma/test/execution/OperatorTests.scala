...
class OperatorTests extends ExecutionTests with ScalaCheckDrivenPropertyChecks {
  ...
  test(s"run: if-then-else") { implicit bc =>
    forAll { (b: Boolean, l: Int, r: Int) =>
      runExprTest(
        s"if $b then $l else $r",
        intReplType,
        (if (b) l else r).toString
      )
    }
  }
    ...
    forAll { (l: BigInt, r: BigInt) =>
      forAll(Gen.choose(-30, -1)) { r: Int =>
        whenever(r < 0) {
          runBadExprTest(
            s"$l ** $r",
            s"PrimitiveException: IntPow: illegal negative power $r given"
          )
        }
      }
    }
  ...
  {
    val func = (l: String, r: String) =>
      s""""${Util.escape(Util.unescape(l) + Util.unescape(r))}""""
  ...
  {
    val func = (l: String, r: String) =>
      toCoomaString(Util.unescape(l) == Util.unescape(r))
  ...
  test("disjunction binds tighter than if-then-else") { implicit bc =>
    runExprTest(
      "if true then false || true else false",
      " : << True : Unit, False : Unit >>",
      toCoomaString(true)
    )
  }
