...
import org.bitbucket.inkytonik.cooma.{
  Backend,
  Compiler,
  Config,
  Main,
...
trait ExecutionTests
  ...
  def runString(name: String, program: String, options: Seq[String])(implicit
      bc: BackendConfig
  ): String = {
    val allArgs =
      Seq("--Koutput", "string") ++ bc.options ++ options :+ "test.cooma"
    runTest(bc.frontend.interpret(name, program, _), allArgs)
  }

  def runFile(program: String, options: Seq[String], args: Seq[String])(implicit
      bc: BackendConfig
  ): String = {
    val allArgs =
      Seq("--Koutput", "string") ++ bc.options ++ options ++ (program +: args)
    runTest(bc.frontend.interpret, allArgs)
  }

  def runMain(program: String, options: Seq[String], args: Seq[String])(implicit
      bc: BackendConfig
  ): String = {
    val allArgs =
      Seq("--Koutput", "string") ++ bc.options ++ options ++ (program +: args)
  ...
  def runREPLTest(cmd: String, input: String, options: Seq[String])(implicit
      bc: BackendConfig
  ): String = {
  ...
  def runREPLOnLine(input: String, options: Seq[String])(implicit
      bc: BackendConfig
  ): String =
    runREPLTest(":paste", input, options)

  def runREPLOnLines(input: String, options: Seq[String])(implicit
      bc: BackendConfig
  ): String =
    runREPLTest(":lines", input, options)

  def runExprTest(code: String, tipe: String, answer: String)(implicit
      bc: BackendConfig
  ): Unit = {
  ...
  def runBadExprTest(code: String, error: String)(implicit
      bc: BackendConfig
  ): Unit = {
    val result1 = runString(name, code, Seq())
  ...
  def runPrimTest(name: String, args: String, tipe: String, answer: String)(
      implicit bc: BackendConfig
  ): Unit =
    runExprTest(s"$name($args)", tipe, answer)

  def runBadPrimTest(name: String, args: String, error: String)(implicit
      bc: BackendConfig
  ): Unit =
