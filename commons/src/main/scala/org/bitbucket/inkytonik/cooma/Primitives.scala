...
trait Primitives extends Database with FileIo with HttpServer {
  ...
  def numArgs(p: Primitive): Int =
    p match {
      ...
      case CapabilityP(_) | DbTableAllP(_, _) | DbTableDeleteP(_, _) |
          HttpServerP(_) | DbTableGetByIdP(_, _) | DbTableInsertP(_, _) |
          DbTableUpdateP(_, _) | FolderReaderReadP(_) | HttpClientP(_, _) |
          ReaderReadP(_) | RunnerRunP(_) | WriterWriteP(_) =>
        1
      case RecConcatP() | RecSelectP() | FolderRunnerRunP(_) |
          FolderWriterWriteP(_) =>
        2
      case UserP(u) =>
        u match {
  ...
  /** @param prim
    *   the `Primitive` object (as instantiated by `capability`)
    * @param rho
    *   the environment
    * @param xs
    *   primitive arguments
    ...
    * @return
    *   result (Cooma value) of primitive application
    */
  def run(
      prim: Primitive,
      rho: Env,
  ...
  /** @param cap
    *   the capability specifier
    * @param rho
    *   the environment
    * @param x
    *   the command-line argument to be passed to the capability constructor
    * @return
  ...
  def capability(cap: String, rho: Env, x: String): ValueR = {
    ...
    def makeCapability(pairs: Vector[(String, Primitive, Int)]): ValueR =
      recR(pairs.map { case (fieldName, primitive, numArgs) =>
        val p = fresh("p")
        def aux(numArgs: Int, args: Vector[String], k0: String): Term = {
          ...
          if (numArgs > 0)
            ...
            letV(p, prmV(primitive, args), appC(idnC(k0), p))
        }
        val k = fresh("k")
        val y = fresh("y")
        fldR(
    ...
    cap match {
      case DatabaseClientRegex(indexStr, spec) =>
        ...
        val tableCaps =
          metadata.tables.map { case Metadata.Table(tablename, _) =>
    ...
    def aux(v: ValueR, side: String): Vector[FldR] =
      isRecR(v) match {
        ...
        case None =>
          errPrim(
            primName(prim),
            s"$side argument $r of record concatenation is non-record"
          )
      }
  ...
  def recSelect(prim: Primitive, rho: Env, r: String, f: String): ValueR = {
    ...
    isRecR(value) match {
      ...
      case None =>
        errPrim(
          primName(prim),
          s"$r is non-record $value, looking for field $f"
        )
    }
  }
  ...
  def lookupVector(rho: Env, name: String): Vector[ValueR] =
    isVecR(lookupR(rho, name)) match {
      ...
      case None =>
        errPrim(
          "LookupVector",
          s"$name is ${lookupR(rho, name)}, expected Vector value"
        )
    }
    ...
    isIntR(lookupR(rho, i)) match {
      case Some(i) =>
        ...
        if (elems.indices contains index)
          ...
          errPrim(
            "VecGet",
            s"vector index out of bounds - size: ${elems.size}, index: $index"
          )
      case _ =>
    ...
    isIntR(lookupR(rho, i)) match {
      case Some(idx) =>
        if (0 <= idx && idx < elems.length)
          ...
          errPrim(
            primName(prim),
            s"vector index out of bounds - size: ${elems.size}, index: $idx"
          )
      case None =>
