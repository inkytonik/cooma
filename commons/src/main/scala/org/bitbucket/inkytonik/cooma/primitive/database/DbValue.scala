...
sealed trait DbValue extends Product {
  ...
  def checkType(tipe: DbType): Boolean =
    (this, tipe) match {
      case (_: DbValue.Boolean, DbType.Boolean) |
          ...
          (DbValue.Null, DbType.Nullable(_)) =>
        true
      case (DbValue.NotNull(value), DbType.Nullable(tipe)) =>
        value.checkType(tipe)
      case _ => false
    }
  ...
  def toCooma(backend: Backend): backend.ValueR = {
    import backend._
    this match {
      case DbValue.Boolean(boolean) =>
        varR(if (boolean) "True" else "False", uniR)
...
object DbValue {
    ...
    def tryNotNull(v: ValueR) =
      isVarR(v) match {
        case Some(("Some", value)) =>
          fromCooma(backend)(value).collect { case v: Atomic => NotNull(v) }
        case _ => None
      }
  ...
  def rowFromCooma(
      backend: Backend
  )(row: backend.ValueR): Option[Seq[(java.lang.String, DbValue)]] = {
    import backend._
    @tailrec
    def aux(
        fields: Seq[FldR],
        out: Seq[(java.lang.String, DbValue)]
    ): Option[Seq[(java.lang.String, DbValue)]] =
      fields match {
        case hd +: tl =>
          isFldR(hd) match {
            case Some((k, v)) =>
