...
object Desugar {
  ...
  def desugar[T <: ASTNode](
    ...
    def appOp(
      ...
      positions.dupPos(
        n,
        App(Sel(Idn(IdnUse(rec)), FieldUse(field)), args.toVector)
      )
      ...
      def relational(
        ...
        analyser.tipe(l) match {
          ...
          case Some(_) =>
            addError(
              n,
              "illegal relational operator, only Int or String supported"
            )
          case None =>
            n
        }
        ...
        case n @ Abs(e) =>
          analyser.tipe(e) match {
            ...
            case Some(t) =>
              addError(
                n,
                s"illegal |${show(t)}|, only Int, String or Vector supported"
              )
            case None =>
              n
          }
        ...
        case n @ Add(l, Concat(), r) =>
          (analyser.tipe(l), analyser.tipe(r)) match {
            ...
            case (Some(t), Some(u)) =>
              addError(
                n,
                s"illegal ${show(t)} ++ ${show(u)}, only String or Vector supported"
              )
            case _ =>
              n
          }
