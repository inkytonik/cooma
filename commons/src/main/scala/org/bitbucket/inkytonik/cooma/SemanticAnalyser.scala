...
class SemanticAnalyser(
      ...
      case Some(t @ VarT(fieldTypes)) =>
        fieldTypes.find(f => f.identifier == c.identifier) match {
          case None =>
            error(
              c,
              s"variant ${c.identifier} not present in matched type ${show(t)}"
            )
          case _ =>
            noMessages
        }
  ...
  def checkConcat(e: Cat, l: Expression, r: Expression): Messages = {
    ...
    (tipe(l), tipe(r)) match {
      ...
      case (Some(t), _) =>
        error(
          l,
          s"expected record or record type, got ${show(l)} of type ${show(t)}"
        )
    }
  }
  ...
  def checkMatch(m: Mat): Messages = {
    val e = m.expression
    val cs = m.cases
    checkMatchCaseNum(e, cs) ++ checkMatchDiscType(
      e
    ) ++ checkCaseExpressionTypes(m)
  }
      ...
      t match {
        case App(Idn(IdnUse("Database")), Vector(RecT(tables))) =>
          tables.forall {
            case FieldType(
                  _,
                  App(Idn(IdnUse("Table")), Vector(RecT(headers)))
                ) =>
              headers.forall {
                case FieldType(_, t) =>
                  ...
                  t match {
                    ...
                    case App(Idn(IdnUse("Option")), Vector(Idn(IdnUse(s)))) =>
                      isBaseType(s)
                    case _ => false
                  }
                case _ => false
        ...
        case App(Idn(IdnUse("HttpServer")), Vector(RecT(endpoints))) =>
          endpoints.forall {
            case FieldType(
                  _,
                  FunT(ArgumentTypes(Vector(ArgumentType(_, StrT()))), StrT())
                ) =>
              true
            case _ => false
          }
  ...
  def checkPrimitive(prm: Prm): Messages = {
    ...
    if (numArgs != expectedNumArgs)
      error(
        prm,
        s"primitive ${show(prm.userPrimitive)} expects $expectedNumArgs arguments got ${numArgs}"
      )
    else
      noMessages
  }
  ...
  object Scope {
    def unapply(n: ASTNode): Boolean =
      n match {
        case _: Body | _: Case | _: Fun | _: BlkDef | _: BlkLet | _: REPLDef |
            _: REPLExp | _: REPLLet =>
          true
        case _ =>
          false
  ...
  /** The "deepest" env of an expression, defined to be the env of the plain
    * expression, and the env of the body of a block expression (recursively).
    * Used when extracting the predef env.
    */
  ...
  def substArgTypes[T](x: String, t: Expression, a: T) = {
    val substArgType =
      rule[Expression] { case Idn(IdnUse(`x`)) =>
        t
      }
  ...
  val appType: App => (Option[Vector[ArgumentType]], Option[Expression]) =
    attr { case app @ App(f, as) =>
      appType(app, tipe(f), as)
    }

  def appType(
      n: Expression,
      ftype: Option[Expression],
      as: Vector[Expression]
  ): (Option[Vector[ArgumentType]], Option[Expression]) =
    ftype match {
  ...
  def appType(
      n: Expression,
      ts: Vector[ArgumentType],
      newts: Vector[ArgumentType],
      t: Expression,
      as: Vector[Expression]
  ): (Option[Vector[ArgumentType]], Option[Expression]) =
    if (as.isEmpty)
      ...
      ts.head match {
        case ArgumentType(Some(IdnDef(x)), TypT()) =>
          appType(
            n,
            substArgTypes(x, as.head, ts.tail),
            newts :+ ts.head,
            substArgTypes(x, as.head, t),
            as.tail
          )
        case _ =>
  ...
  def fieldType(fieldTypes: Vector[FieldType], f: String): Option[Expression] =
    fieldTypes.find { case FieldType(i, t) =>
      i == f
    } match {
      ...
      case n @ App(Idn(f), as) =>
        appResType(n) match {
          case Some(t) if t == typT =>
            ...
            optArgsAndBody.flatMap { case (das, body) =>
      ...
      case tree.parent.pair(a: Expression, Prm(p, as)) =>
        ...
        userPrimitiveType(p) match {
          case FunT(ArgumentTypes(ts), _)
              if (as.length == ts.length) && (ts.length >= argnum) =>
  ...
  def funBound(t: FunT, u: FunT, direction: BoundDirection): Option[FunT] = {
    val (FunT(ArgumentTypes(tats), trt), FunT(ArgumentTypes(uats), urt)) =
      (t, u)
    if (tats.length != uats.length)
      ...
      checkReturn(trt, urt) match {
        case Some(rt) =>
          ...
          aux(tats.zip(uats), Vector.empty).map(ats =>
            FunT(ArgumentTypes(ats), rt)
          )
        case None =>
          None
      }
