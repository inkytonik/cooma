...
trait Backend extends Primitives {
  ...
  /** When evaluating a program using Truffle, what we get as an output are
    * org.graalvm.polyglot.Value values instead of ValueRs, therefore, we need
    * to have this second output type. For the reference interpreter, this type
    * will point to the ValueR and in the case of the truffle, to
