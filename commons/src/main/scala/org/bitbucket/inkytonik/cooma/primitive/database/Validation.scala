...
object Validation {
  ...
  def validateColumn(expected: Column, actual: SqlColumn): Seq[String] = {
    ...
    val typeError = {
      ...
      if (compatible) Seq.empty
        ...
        Seq(
          s"column '$name' has type '$aType', incompatible with ${tipe.atomic.toCoomaString}"
        )
    }
    val nullableError =
    ...
    val errors =
      for {
        ...
        _ <- {
          // validate columns
          val errors =
            expectedColumns.flatMap { case expected @ Column(name, _) =>
  ...
  def validateTables(
    ...
    val tablesWithErrors =
      tables.flatMap { case table @ Table(name, _) =>
    ...
    if (tablesWithErrors.nonEmpty) {
      ...
      val message =
        tablesWithErrors
          .flatMap { case (name, errors) =>
            s"$t$name:" +: errors.map(e => s"$t$t$e")
          }
