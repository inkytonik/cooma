...
lazy val reference = (project in file("reference"))
  .settings(
    commonsettings,
    Compile / mainClass := Some("org.bitbucket.inkytonik.cooma.Main")
  ) dependsOn (commons)
...
lazy val truffle = (project in file("truffle"))
  .settings(
    ...
    // sbt-rats
    ratsMainModule := Some(
      (Compile / scalaSource).value / "org" / "bitbucket" / "inkytonik" / "cooma" / "truffle" / "nodes" / "term" / "CoomaTermParser.rats"
    ),
    compileOrder := CompileOrder.Mixed,
    libraryDependencies ++= Seq(
...
lazy val buildComponent =
  taskKey[Unit]("Generates the component jar for GraalVM installation.")

lazy val installGraalVMComponent =
  taskKey[Unit]("Installs the generated component in the GraalVM")
